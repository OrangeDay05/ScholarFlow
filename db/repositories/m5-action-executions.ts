...
async function ownedProjectId(db: D1Database, ownerUserId: string, requestedProjectId: string): Promise<string> {
  if (!requestedProjectId || requestedProjectId === "demo") throw new M5ActionExecutionError("PROJECT_NOT_FOUND", "缺少明确的项目上下文，请先选择项目。");
  const row = await db.prepare(`SELECT id FROM projects WHERE id = ? AND owner_user_id = ? AND status = 'active'`)
