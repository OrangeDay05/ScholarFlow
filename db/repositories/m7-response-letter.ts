...
export async function createM7ResponseLetterDocx(actor: M3Actor, requestedProjectId: string, revisionTaskIds: string[], storage: StorageAdapter = getMaterialStorageAdapter()) {
  ...
  for (const id of ids) {
    const row = await db.prepare(
      `SELECT t.result_version_id, t.verification_note, t.response_strategy, t.decision_reason, t.incomplete_experiment_warning, c.reviewer_label, c.comment_number, c.content,
    ...
    ).bind(id, actor.userId, project.id).first<{ result_version_id: string | null; verification_note: string | null; response_strategy: string; decision_reason: string | null; incomplete_experiment_warning: string | null; reviewer_label: string; comment_number: string; content: string; response: string | null }>();
    if (!row?.response || !row.result_version_id) throw new M7RevisionError("CONFIRMATION_REQUIRED", "回复信仅能包含已验证返修和用户确认的回复。");
    const warning = row.incomplete_experiment_warning ? `\n\n未完成实验警告：\n${row.incomplete_experiment_warning}` : "";
    const reason = row.decision_reason ? `\n回应理由：${row.decision_reason}` : "";
    versionIds.push(row.result_version_id); sections.push({ title: `${row.reviewer_label} — Comment ${row.comment_number}`, content: `审稿意见：\n${row.content}\n\n回应策略：${row.response_strategy}${reason}\n\n回复：\n${row.response}\n\n修改验证：\n${row.verification_note ?? "已验证"}${warning}\n\n采用版本：${row.result_version_id}` });
  }
