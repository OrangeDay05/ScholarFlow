...
export const modelCapabilityVersions = sqliteTable(
  "model_capability_versions",
  {
    id: text("id").primaryKey(),
    providerId: text("provider_id").notNull().references(() => modelProviders.id, { onDelete: "cascade" }),
    modelId: text("model_id").notNull().references(() => providerModels.id, { onDelete: "cascade" }),
    ...
    supportedThinkingModesJson: text("supported_thinking_modes_json").notNull().default("[]"),
    supportedReasoningEffortsJson: text("supported_reasoning_efforts_json").notNull().default("[]"),
    ...
    supportsThinkingToolCalls: integer("supports_thinking_tool_calls", { mode: "boolean" }).notNull().default(false),
    supportsJsonOutput: integer("supports_json_output", { mode: "boolean" }).notNull().default(false),
    supportsVision: integer("supports_vision", { mode: "boolean" }).notNull().default(false),
    contextWindow: integer("context_window").notNull(),
    maxOutputTokens: integer("max_output_tokens").notNull(),
    supportedParametersJson: text("supported_parameters_json").notNull().default("[]"),
    ignoredParametersJson: text("ignored_parameters_json").notNull().default("[]"),
    lifecycleStatus: text("lifecycle_status", { enum: ["DISCOVERED", "TESTING", "ACTIVE", "DEPRECATED", "RETIRED", "DISABLED"] }).notNull(),
  ...
  (table) => [uniqueIndex("model_capability_model_version_uq").on(table.modelId, table.capabilityVersion), index("model_capability_lifecycle_idx").on(table.lifecycleStatus)],
);

export const providerCatalogSyncs = sqliteTable(
  "provider_catalog_syncs",
  {
    id: text("id").primaryKey(),
    providerId: text("provider_id").notNull().references(() => modelProviders.id, { onDelete: "cascade" }),
    status: text("status", { enum: ["SUCCEEDED", "FAILED"] }).notNull(),
    discoveredModelIdsJson: text("discovered_model_ids_json").notNull().default("[]"),
    errorCode: text("error_code"),
    syncedAt: text("synced_at").notNull(),
    createdAt: text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
  },
  (table) => [index("provider_catalog_sync_provider_idx").on(table.providerId, table.syncedAt)],
);

export const agentRoleModelConfigs = sqliteTable(
  "agent_role_model_configs",
  {
    ...
    modelId: text("model_id").notNull().references(() => providerModels.id, { onDelete: "restrict" }),
    credentialType: text("credential_type", { enum: ["PLATFORM_CREDENTIAL", "USER_CREDENTIAL"] }).notNull(),
    credentialReference: text("credential_reference").notNull(),
    thinkingMode: text("thinking_mode", { enum: ["DISABLED", "ENABLED"] }).notNull(),
    reasoningEffort: text("reasoning_effort", { enum: ["LOW", "MEDIUM", "HIGH", "MAX"] }),
    maxOutputTokens: integer("max_output_tokens").notNull(),
    timeoutMs: integer("timeout_ms").notNull(),
    perTurnBudget: integer("per_turn_budget").notNull(),
    toolsAllowed: integer("tools_allowed", { mode: "boolean" }).notNull().default(false),
    fallbackConfigId: text("fallback_config_id"),
    status: text("status", { enum: ["DRAFT", "ACTIVE", "DISABLED"] }).notNull().default("DRAFT"),
    ...timestamps(),
  },
  (table) => [index("agent_role_config_owner_project_idx").on(table.ownerUserId, table.projectId, table.agentRole)],
);

export const modelPricingVersions = sqliteTable(
  "model_pricing_versions",
  {
    ...
    inputCacheHitPrice: text("input_cache_hit_price").notNull(),
    inputCacheMissPrice: text("input_cache_miss_price").notNull(),
    outputPrice: text("output_price").notNull(),
    currency: text("currency").notNull(),
    unit: text("unit").notNull(),
    effectiveFrom: text("effective_from").notNull(),
    effectiveTo: text("effective_to"),
    sourceUpdatedAt: text("source_updated_at").notNull(),
    status: text("status", { enum: ["ACTIVE", "SUPERSEDED", "DISABLED"] }).notNull(),
    createdAt: text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
  },
  (table) => [uniqueIndex("model_pricing_model_version_uq").on(table.modelId, table.pricingVersion)],
);

export const resolvedModelConfigSnapshots = sqliteTable(
  "resolved_model_config_snapshots",
  {
    id: text("id").primaryKey(),
    ownerUserId: text("owner_user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
    projectId: text("project_id").notNull().references(() => projects.id, { onDelete: "cascade" }),
    taskId: text("task_id").references(() => aiTasks.id, { onDelete: "cascade" }),
    ...
    thinkingMode: text("thinking_mode", { enum: ["DISABLED", "ENABLED"] }).notNull(),
    reasoningEffort: text("reasoning_effort", { enum: ["LOW", "MEDIUM", "HIGH", "MAX"] }),
    effectiveParametersJson: text("effective_parameters_json").notNull(),
    ignoredParametersJson: text("ignored_parameters_json").notNull().default("[]"),
    credentialType: text("credential_type", { enum: ["PLATFORM_CREDENTIAL", "USER_CREDENTIAL"] }).notNull(),
    credentialReference: text("credential_reference").notNull(),
  ...
  (table) => [index("resolved_model_snapshot_task_idx").on(table.ownerUserId, table.projectId, table.taskId)],
);

export const providerRunRecords = sqliteTable(
  "provider_run_records",
  {
    ...
    reasoningContentProduced: integer("reasoning_content_produced", { mode: "boolean" }).notNull().default(false),
    reasoningContentCharacters: integer("reasoning_content_characters").notNull().default(0),
    toolCallNamesJson: text("tool_call_names_json").notNull().default("[]"),
...
export const figureCodeVersions = sqliteTable(
  "figure_code_versions",
  {
    ...
    projectId: text("project_id").notNull().references(() => projects.id, { onDelete: "cascade" }),
    figureProjectId: text("figure_project_id").notNull().references(() => figureProjects.id, { onDelete: "cascade" }),
    language: text("language", { enum: ["python", "r", "mermaid"] }).notNull().default("python"),
    engine: text("engine", { enum: ["matplotlib", "seaborn", "plotly", "ggplot2", "graphviz", "controlled_svg"] }).notNull().default("matplotlib"),
