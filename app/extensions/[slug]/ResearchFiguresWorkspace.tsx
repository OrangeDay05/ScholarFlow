  ...
  return (
    <div className={styles.workspace}>
      <aside className={styles.controls}>
        <PanelTitle kicker="DATA & FIGURE" title="数据与图型" badge="M8.1" />
        ...
        <div className={styles.catalog}>{M8_FIGURE_CATALOG.map((group) => <section key={group.group}><h3>{group.label}</h3><div>{group.items.map((type) => {
          ...
          return <button className={spec.chartType === type ? styles.activeType : undefined} disabled={!enabled} key={type} onClick={() => enabled && changeType(type as M8ImplementedFigureType)} type="button">{chartLabels[type]}{!enabled ? <small>M8.2</small> : null}</button>;
        })}</div></section>)}</div>
...
function MappingControls({ columns, spec, update }: { columns: M8ColumnSchema[]; spec: M8StatisticalFigureSpec; update: (field: string, value: string) => void }) {
  ...
  const fields = spec.chartType === "scatter" || spec.chartType === "line" ? [{ key: "x", label: "X 字段" }, { key: "y", label: "Y 字段" }, ...(spec.chartType === "line" ? [{ key: "group", label: "分组（可选）" }] : [])] : [{ key: "category", label: "分类字段" }, { key: "value", label: "数值字段" }];
...
function defaultMapping(type: M8ImplementedFigureType, columns: M8ColumnSchema[]): M8StatisticalFigureSpec["mapping"] { const numeric = columns.find((column) => column.type === "number")?.name ?? ""; const secondNumeric = columns.filter((column) => column.type === "number")[1]?.name ?? numeric; const category = columns.find((column) => column.type === "string" || column.type === "date")?.name ?? columns[0]?.name ?? ""; if (type === "scatter") return { x: numeric, y: secondNumeric }; if (type === "line") return { x: category, y: numeric }; return { category, value: numeric }; }
