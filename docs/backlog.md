# Backlog

## READY

- M5 外部验收：用户显式提供专用测试凭据后，验证一次 OpenAI/DeepSeek 连接与有界任务；不得使用生产 Key。

## IN_PROGRESS

- M10：管理员、指标、全链路测试、安全、性能和发布候选。

## DONE

- M0-M2：工程基线、视觉基线与完整可点击原型。
- V0.4.2：六个独立增量业务页面与功能开关。
- M3 基线：基础数据模型、所有者隔离 API 与可关闭 Adapter。
- V0.4.3：双模型生成、独立审阅、用户采纳、追加修订与最终验证 Mock。
- M3-PD：三个最低创建问题、四种诊断入口、单问题引导、字段状态/来源、停止条件、任务就绪与诊断版本 Mock。
- M3-MO：五种模型角色、三种编排模式、平台/用户凭据、安全边界、执行前披露、冲突与八种失败状态 Mock。
- M3 增量 QA：专项测试、SSR/HTTP、类型、Lint、构建与正式审核报告。
- M4-B1—B7：V0.5.1 基线、五起点、渐进式诊断、任务/审阅、隐私保真、PPT、模型/BYOK 元数据、隔离测试与审核包。
- M4-H1/H2：本地 D1 基线与 CORE-01 真实认证。
- M5-B1：六 Skill 统一上下文/结果、Provider Adapter、凭据解析、预算、超时和任务恢复边界。
- M5-B2A：双页签、Conversation Agent 前端基础、六 Skill 默认 Prompt、ToolIntent、摘要、Action Proposal 和用户确认门。
- M5-B2B：长期会话、消息和派生摘要 Repository/API、幂等、归档与所有者隔离；0007 仅生成未执行。
- M5-B2C：ToolIntent、Action Proposal、用户决定和恢复状态 Repository/API；确认只到 `READY_TO_QUEUE`，0008 仅生成未执行。
- M5-B2D：会话分页、追加式压缩计划、连续摘要来源、恢复快照、幂等失败重试与权限完整回归；无新迁移。
- M5-B3A：Storage Adapter、本地 Miniflare R2、真实上传、安全文件探测、不可覆盖对象记录、0006、隔离与失败补偿。
- M5-B3B—B5B：全格式解析、项目知识检索、加密凭据、Provider/Skill Adapter、有界 Runner、任务结果与候选版本。
- M6：Claim/Evidence、严格核验、高级审阅、Data Availability、投稿准备、真实 DOCX 和 Word 打开验证。
- M7：决定信、Comment、Revision Task、Response Draft、修改核对和 Response Letter DOCX。
- M8.1：CSV/表格、五类核心统计图、本地受信任 Runner、真实 PNG 与全链路追溯。
- M8.2：完整 17 类统计图目录，含直方图、密度图、点图、误差线、森林图、气泡图、回归、面积、热图、相关矩阵、分面与多面板图。
- M8.3：PNG/SVG/PDF/TIFF 多格式资产、安全签名检查、正确 MIME 与 hash/大小资产清单。
- M8.4：五类参数化概念图、受控 SVG、Mermaid 结构代码展示、不可变版本与所有者隔离资产。
- M9：13 种场景、来源与版本绑定、讲者备注、问答准备、真实 PPTX、图件嵌入和打开验证。

## M8 以后

- M10：管理员、运营与发布候选。
- M11：只有用户明确授权后才可生产部署。

未经用户后续批准不得启动 M8 以后能力或生产部署。
