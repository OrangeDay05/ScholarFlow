...
| I-017 / M5-FU-01 | `BUDGET_PAUSED` 当前恢复决策仍会返回 `RESUME`，不符合必须等待用户增加预算或重新确认的规则 | M5-B1 审核 | 高 | Root Agent | OPEN | 最迟在 M5-B4 真实 Runner 前改为显式 `WAIT_FOR_USER` / `PAUSED`，并增加预算确认恢复测试；M5-B2A 只登记、不实现。 |
| I-018 / M5-FU-02 | Provider Adapter 尚无统一 `M5ProviderError` 和安全错误字段 | M5-B1 审核 | 高 | Root Agent | OPEN | 最迟在 M5-B3 真实 Provider Adapter 前实现统一错误类型、`retryable/provider/statusCode/retryAfter/safeMessage` 及九类错误映射；M5-B2A 只登记、不实现。 |
