...
| M5-B3B | M5 | TXT、CSV、BibTeX、RIS 解析 | Root Agent | `app/lib/material-parsers/**`、`app/api/m5/**/parse`、`db/**`、`drizzle/0009*`、测试与文档 | M5-B3A | 不可变原文件、Storage Adapter | ParseRun、Chunk、行/记录/字段来源位置与失败状态 | 重解析不覆盖；幂等重放；失败不标 success；0000→0009 可回放 | DONE | 大文本耗尽资源、来源位置丢失、重复解析覆盖 | 5 MB/10,000 记录上限；四种解析器专项测试通过；原对象只读 |
