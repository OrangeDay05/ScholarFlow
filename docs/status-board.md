...
- 当前状态：WAITING_FOR_REVIEW（M5-B2C 操作提案确认门持久化）
- 活动工程：`site/`（唯一活动工程）
...
- 当前批次：M5-B2C ToolIntent、Action Proposal、用户决定与恢复状态持久化已完成，等待审核
...
审核 M5-B2C 后再进入会话压缩与恢复回归；已确认 Action Proposal 仅进入 `READY_TO_QUEUE`，不创建或执行 AI Task，文件仍只到 `AWAITING_PARSE`。
