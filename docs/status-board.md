...
- 当前状态：M5_IN_PROGRESS（M5 收尾与外部凭据验收）
- 活动工程：`site/`（唯一活动工程）
...
- M5-B5：PARTIAL — 有界 Runner、独立审阅、部分成功保留、预算暂停/取消、任务结果和候选版本追加持久化已完成；真实凭据端到端验收待完成。
...
继续 M5 收尾：真实供应商端到端验收需要用户显式提供测试凭据；不以缺少凭据阻止后续内部 M6/M7 代码批次，但不得将该外部验收标记通过。
