...
| R-023 | 本地 Schema 实际状态与 migration 台账不一致，重复执行会因表已存在失败 | 高 | 高 | M4-B1 冻结现状；只在空隔离库验证迁移链；现有库先预检再协调台账，不盲目重建 | Root Agent | OPEN |
| R-024 | 对敏感材料统一删除或脱敏会改变样本、实验条件、时序、数值精度或参与者关系 | 中 | 高 | 按直接/间接标识符、敏感属性、必要变量、正文、伦理、版权分类；每份处理副本记录保真检查和阻断原因 | Root Agent | OPEN |
| R-025 | PPT 可用性按学历或正式答辩场景硬编码，排除课程、课堂、文献和小组汇报 | 中 | 中 | 使用 V0.5.1 的 13 个场景枚举；只按材料就绪度和内容真实性门控 | Root Agent | OPEN |
| R-026 | BYOK 元数据实现意外接收、回显或记录明文 Key | 中 | 高 | M4 API 拒绝明文字段；只保存掩码、密文引用和角色/范围元数据；真实加密与连接延后 M5 | Root Agent | OPEN |
